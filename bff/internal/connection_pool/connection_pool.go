...
type ConnectionPool struct {
	mu          sync.Mutex
	connections map[entity.ConnectionID]*Connection
...
func New() *ConnectionPool {
	return &ConnectionPool{
		connections: make(map[entity.ConnectionID]*Connection),
		maxHeap:     ConnectionHeap{},
	}
}
...
func (cp *ConnectionPool) UpdateFreeSpace(connectionID entity.ConnectionID, size uint64) {
	cp.mu.Lock()
	defer cp.mu.Unlock()
...
func (cp *ConnectionPool) GetConnection(connectionID entity.ConnectionID) *Connection {

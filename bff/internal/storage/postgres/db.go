...
func (db *DB) SaveFileInfo(ctx context.Context, fm *entity.FileMetadata) error {
...
func (db *DB) GetFileInfo(ctx context.Context, fileID uuid.UUID) (*entity.FileMetadata, error) {
...
func (f *File) Marshal(fm *entity.FileMetadata) *File {
	f = &File{
		ID: fm.ID,
		Info: &FileInfo{
			Name:     fm.Name,
...
func (f *File) Unmarshal() *entity.FileMetadata {
	...
	for i, storageNum := range f.Info.Storages {
		fileMetadata.Storages[entity.FilePart(i)] = entity.ConnectionID(storageNum)
	}
